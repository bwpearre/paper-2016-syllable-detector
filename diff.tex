...
%DIF DEL ../paper-2016-syllable-detector-last/main.tex   Mon Jul 17 14:42:20 2017
%DIF ADD main.tex                                        Mon Jul 17 15:45:53 2017
...
%DIF 94c94
%DIF < \renewcommand{\subsubsection}[1]{\paragraph{#1}}
%DIF -------
%\renewcommand{\subsubsection}[1]{\paragraph{#1}} %DIF > 
%DIF -------
...
\\
Ben Pearre\textsuperscript{1,\DIFdelbegin \DIFdel{a,}\DIFdelend \textcurrency},
L.~Nathan Perkins\textsuperscript{1\DIFdelbegin \DIFdel{,b}\DIFdelend },
Jeffrey E.~Markowitz\textsuperscript{2\DIFdelbegin \DIFdel{,c}\DIFdelend },
Timothy J.~Gardner\textsuperscript{1\DIFdelbegin \DIFdel{,d}\DIFdelend }
\\
\bigskip
...
\DIFdelbegin \DIFdel{Author \textsuperscript{a}~contributed the original MATLAB and LabVIEW implementations and most of the manuscript.
...
%DIF > Author \textsuperscript{a}~contributed the original MATLAB and LabVIEW %implementations and most of the manuscript.
%DIF > \textsuperscript{b}~contributed the Swift implementation and its description herein, %helped debug the training code, and measured the timing for most of the detectors.
%DIF > textsuperscript{c}~wrote several types of filter-bank software against which we %compared, made suggestions for the neural network detector, and reformatted the %MATLAB code for distribution.
%DIF > \textsuperscript{d}~provided a stream of suggested techniques, metrics, and goals.
\DIFaddbegin 

\DIFaddend % Additional Equal Contribution Note
...
\begin{abstract}
  ...
  well under 1\% and 0.005\% respectively. The detector can run on a
...
second. The song may be repeated hundreds of times per day, and is almost
identical each time.  Several brain areas reflect this consistency
in highly stereotyped neural firing patterns, which makes
...
\begin{figure}
  ...
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Fig1}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF > \includegraphics[width=\textwidth]{Fig1}
...
\begin{figure}
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Fig2}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF > \includegraphics[width=\textwidth]{Fig2}
  % 9x10 inches
...
\begin{figure}
  % Width 10, height 4, font 100%
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Fig3}
%DIFDELCMD <   %%%
%DIF < \includegraphics[width=\textwidth]{accuracies-2hidden}
  \DIFdelendFL %DIF > \includegraphics[width=\textwidth]{Fig3}
...
\begin{table}
  % replot_accuracies_concatanated('tex', true) for confusion_log_perf.txt (1.5 ms).
  \begin{tabular}{lc|cc}
    Bird & Target time (ms) & \% True positives & \% False positives \\ \hline
lny64 & 	150 $[t^*_1]$ & 	99.66 & 	0.0026 \\ % /Volumes/Data/song/lny64/confusion_log_perf.txt
      & 	200 $[t^*_2]$ & 	99.75 & 	0.00052 \\ %                                               
      & 	250 $[t^*_3]$ & 	99.79 & 	0.00051 \\ %                                               
      & 	300 $[t^*_4]$ & 	99.70 & 	0.00077 \\ %                                               
      & 	350 $[t^*_5]$ & 	99.73 & 	0.00022 \\ %                                               
      & 	400 $[t^*_6]$ & 	99.52 & 	0.0010 \\ %                                                
lny42 & 	380 & 	99.09 & 	0.0011 \\ % /Volumes/Data/song/lny42/confusion_log_perf.txt          
lny46 & 	140 & 	99.11 & 	0.0036 \\ % /Volumes/Data/song/lny46/confusion_log_perf.txt          
lny4rb & 	250 & 	99.37 & 	0.0026 \\ % /Volumes/Data/song/lny4rb/confusion_log_perf.txt        
lr12 & 	180 & 	99.52 & 	0.0033 \\ % /Volumes/Data/song/lr12/all/confusion_log_perf.txt        
lr13 & 	300 & 	99.83 & 	0.0013 \\ % /Volumes/Data/song/lr13/all/confusion_log_perf.txt        
lr28 & 	140 & 	99.73 & 	0.0028 \\ % /Volumes/Data/song/lr28/all/confusion_log_perf.txt        
lr77 & 	320 & 	99.58 & 	0.0028  % /Volumes/Data/song/lr77/all/confusion_log_perf.txt          
  \end{tabular}
...
\begin{figure}
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Fig4}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF > \includegraphics[width=\textwidth]{Fig4}
...
the variability is surprisingly low, and is summarised \DIFdelbegin \DIFdel{here with 95\%
confidence intervals (with n=6):
}%DIFDELCMD < \vspace{8pt}\par\noindent
%DIFDELCMD < %%%
...
\begin{table}
  \DIFaddendFL \begin{tabular}{r|cc}
    Detector & Latency & Jitter \\ 
    \hline   Ideal & $-0.8\pm 0.3$ ms & $2.1\pm 0.1$ ms \\
    Swift+serial & $0.0\pm 0.6$ ms & $2.0\pm 0.1$ ms
  \end{tabular}
  \DIFdelbeginFL %DIFDELCMD < \vspace{8pt}\par\noindent
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \caption{\DIFaddFL{Latency and jitter variability (95\% confidence) for lny64's six test syllables.}}
  \label{table:TimingVsSyllable}
\end{table}

\DIFaddend The negative latency is due to the way in which the network responds
...
\begin{figure}
  \begin{center}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=8cm]{Fig5}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF > \includegraphics[width=8cm]{Fig5}
  \end{center}
...
$t_\mathrm{fft}=1.5$ ms.  Results are \DIFdelbegin \DIFdel{as follows:
}%DIFDELCMD < \vspace{8pt}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{shown in Table~\ref{table:TimingVsDetector} and }\fig{fig:TimingVsDetector}\DIFadd{.
}\fig{fig:timing} \DIFadd{gives a more detailed view of what the timing curves
look like for the five implementations of our detector on lny64's $t^*_4$.
}

\begin{table}
  \DIFaddendFL \begin{tabular}{l|c|c|c|c}
    & \multicolumn{2}{c}{$\delta$-syllable} & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c}{Bird: $t^*_4$} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c}{lny64: $t^*_4$} \DIFaddendFL \\
  ...
  \DIFdelbeginFL %DIFDELCMD < \vspace{8pt}\par\noindent
%DIFDELCMD < %%%
\DIFdelFL{These data are also plotted in }%DIFDELCMD < \fig{fig:TimingVsDetector}%%%
\DIFdelFL{, and
}%DIFDELCMD < \fig{fig:timing} %%%
\DIFdelFL{gives a more detailed view of what the timing curves
look like for the five implementations of our detector on $t^*_4$.
}\DIFdelendFL \DIFaddbeginFL \caption{\DIFaddFL{Latency and jitter for each of our detector implementations on the synthetic $\delta$-syllable and on syllable $t^*_4$ from lny64.}}
...
\begin{figure}
  \begin{center}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=8cm]{Fig6}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF > \includegraphics[width=8cm]{Fig6}
  \end{center}
...
\begin{figure}
  \begin{center}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=8cm]{Fig7}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF > \includegraphics[width=8cm]{Fig7}
  \end{center}
...
\DIFdelbegin \DIFdel{This work was funded by NIH grants
...
}\DIFdelend %DIF >   This work was funded by NIH grants
%DIF > 5R01NS089679-02 and 5U01NS090454-02.  The funders had no role in study %design, data collection and analysis, decision to publish, or preparation of the %manuscript.
...
%\item[Licenses:]
%  All software is licensed under Open-Source-Initiative--approved licenses. Please see the software packages for individual choices.
...
\item[Runtime:]\hfill
  ...
  \item[MATLAB:]\hfill
    \begin{trivlist}
    \item {\tt https://github.com/gardner-lab/syllable-detector-matlab}
  ...
  \item[Swift:]\hfill
    \begin{trivlist}
    \item {\tt https://github.com/gardner-lab/syllable-detector-swift}

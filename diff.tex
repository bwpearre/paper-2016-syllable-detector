...
%DIF DEL ../pcb/main.tex   Wed Sep  7 17:01:00 2016
%DIF ADD main.tex          Thu Sep 15 12:34:06 2016
...
\begin{abstract}
  ...
  well under \DIFdelbegin \DIFdel{0.5\% and 0.01}\DIFdelend \DIFaddbegin \DIFadd{1\% and 0.001}\DIFaddend \% respectively. The detector can run on a
...
brain areas concerned with the \DIFdelbegin \DIFdel{acquisition and learning }\DIFdelend \DIFaddbegin \DIFadd{learning and production }\DIFaddend of song.  For
...
disruptions.  \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Charlesworth2011learning} }%DIFAUXCMD
showed that the song
...
}\DIFaddend \cite{Warren2011} showed that that AFP transfers \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the error }\DIFaddend signal to
...
Despite the power and versatility of \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{vocal feedback }\DIFaddend experiments, there
is no standard syllable detector.  \DIFdelbegin \DIFdel{Considerations }\DIFdelend \DIFaddbegin \DIFadd{Desiderata }\DIFaddend include:
 \begin{description} 
...
\item Accurate: false \DIFdelbegin \DIFdel{positive rates under 0.02}\DIFdelend \DIFaddbegin \DIFadd{negative rates under 1}\DIFaddend \% and false \DIFdelbegin \DIFdel{negative rates under 0.5}\DIFdelend \DIFaddbegin \DIFadd{positive rates under 0.001}\DIFaddend \% for a variety of syllables.  Balancing these rates depends on a single relative-cost parameter.
...
We begin with a few hundred recordings of a given bird's song, \DIFdelbegin \DIFdel{in this case }\DIFdelend \DIFaddbegin \DIFadd{as well
...
as well as more challenging.  Recordings were }\DIFaddend made inside a
sound-isolating chamber in which was mounted a recording microphone
(Audio-Technica AT803)\DIFdelbegin \DIFdel{.  The songs are
}\DIFdelend \DIFaddbegin \DIFadd{, using methods similar to those described in \mbox{%DIFAUXCMD
\cite{Tchernichovski2011SAP}}%DIFAUXCMD
, Chapter 2.  The songs were
}\DIFaddend time-aligned as described in \cite{Poole2012}.
...
\begin{figure}
  %DIF >  Width: 8, Height: 3, Fonts scale: 120%
  \includegraphics[width=\textwidth]{Fig1}
  \caption{\DIFaddbeginFL \DIFaddFL{The spectrogram of a typical song, used as the example
    throughout this paper.  }\DIFaddendFL This image was made by superposing the
...
}\DIFdelend \DIFaddbegin \DIFadd{With inputs well outside the space on which a neural network has been
...
The network is trained using MATLAB's neural network toolbox\DIFaddbegin \DIFadd{, with Scaled Conjugate Gradient (}{\tt \DIFadd{trainscg}}\DIFadd{)}\DIFaddend . We tried
...
\DIFdelbegin \DIFdel{We used the Levenburg-Marquardt algorithm, which is the MATLAB
...
%DIFDELCMD < %%%
\DIFdelend \subsubsection{Computing optimal output thresholds}
...
are flat, \DIFdelbegin \DIFdel{hillclimbing is ineffective.  Random-restart hillclimbing
would work, but }\DIFdelend \DIFaddbegin \DIFadd{we use }\DIFaddend a brute-force \DIFdelbegin \DIFdel{search }\DIFdelend \DIFaddbegin \DIFadd{linear search, which }\DIFaddend requires fractions of a
second. For the results presented here, we have used
\DIFdelbegin \DIFdel{$C_n=1$ and
}\DIFdelend $\Delta t=10$ ms\DIFaddbegin \DIFadd{, and arbitrarily set $C_n=1$}\DIFaddend .
...
operating system daemon processes but no \DIFdelbegin \DIFdel{user loads }\DIFdelend \DIFaddbegin \DIFadd{loads from user processes}\DIFaddend .
...
implementations, the convenience may outweigh the \DIFdelbegin \DIFdel{slight }\DIFdelend timing
...
\DIFdelbegin \DIFdel{Similar to the Swift implementation}\DIFdelend \DIFaddbegin \DIFadd{As with the Swift detector}\DIFaddend , it is also possible to modify the
...
We \DIFaddbegin \DIFadd{have used this detector for song alignment and feedback experiments
...
\DIFadd{We }\DIFaddend divided the data into training and test sets.  For the results
reported in the following section, the training set consisted of 1000
instances of the song of one bird\DIFaddbegin \DIFadd{, and 2000 song-length samples of
non-song (cage noise, calls, other birds' song)}\DIFaddend .  Our test set
consisted of an additional 1818 iterations of the bird's song \DIFaddbegin \DIFadd{and 3636
song-length samples of non-song}\DIFaddend .

The MATLAB neural network toolbox further divides our ``training'' set
into internal training, validation, and test sets.  \DIFdelbegin \DIFdel{We did not stray
from the
toolbox 's default values, and do not discuss MATLAB's
internal subdivision of our training set .  Because the dataset
}\DIFdelend \DIFaddbegin \DIFadd{Because we do our
...
\DIFadd{Because the data set }\DIFaddend consists of temporally aligned songs, ground
...
defining accuracy on a per-song basis, such that a \DIFdelbegin \DIFdel{song }\DIFdelend \DIFaddbegin \DIFadd{song-length sample }\DIFaddend without the
target syllable counted as a single true negative.  Computing the
optimal output thresholds on a per-frame \DIFaddbegin \DIFadd{rather than a per-song }\DIFaddend basis resulted in higher
thresholds and thus a \DIFdelbegin \DIFdel{lower }\DIFdelend \DIFaddbegin \DIFadd{slight reduction in both the }\DIFaddend false-positive \DIFdelbegin \DIFdel{rate, with minimal
consequences to the false-negative rate, }\DIFdelend \DIFaddbegin \DIFadd{and true-positive rates (reversible by increasing $C_n$), }\DIFaddend while also providing a valid
definition of false-positive rate for data streams that had not been
segmented into \DIFdelbegin \DIFdel{songs}\DIFdelend \DIFaddbegin \DIFadd{song-sized chunks and thus allowing easy interpretation
of false-positive rates from calls, cage noise, etc}\DIFaddend .
...
We evaluate the time taken from the \DIFdelbegin \DIFdel{presentation of }\DIFdelend \DIFaddbegin \DIFadd{complete presentation of each
instance of }\DIFaddend the target syllable to the firing of the detector's TTL
pulse.  \DIFaddbegin \DIFadd{For example, if the target trigger point is at 200 ms with
...
\DIFaddend While playing the audio test file from any audio playback device, \DIFaddbegin \DIFadd{we
used }\DIFaddend the TTL output from the ground-truth channel of the audio output
\DIFdelbegin \DIFdel{may be used }\DIFdelend as the trigger pulse for an oscilloscope, and compared \DIFaddbegin \DIFadd{it }\DIFaddend to the TTL
pulse produced by the detector\DIFdelbegin \DIFdel{implementation}\DIFdelend , which sees only the birdsong channel
...
%DIF <  Use to say: "Mean latency is then given as the halfway point of that detection 
%DIF <  sigmoid."
%DIF < 
%DIF <  I don't think this description matches the calculation, this description
%DIF <  seems to better match a "median" latency. The mean latency is calculated
%DIF <  as an average latency of all detection events, which is the same as 
%DIF <  fitting a normal distribution to the detection events and taking the mean
%DIF <  of the normal distribution (the MLE estimator is the same as the sample mean).
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Mean latency is then given as the average time between the canonical
signal and the corresponding detection event.  It is a helpful number,
but not a critical one, since \DIFaddbegin \DIFadd{in zebra finch }\DIFaddend a detector with high but
constant latency can often be trained to trigger at a point somewhat
before the true moment of interest \DIFdelbegin \DIFdel{,
especially given the stereotypy of zebra finch song}\DIFdelend \DIFaddbegin \DIFadd{(for example, if triggering should
occur at the very beginning of a syllable, the detector's time window
may include previous syllable)}\DIFaddend .  Often more important is \DIFdelbegin \DIFdel{latency jitter: how much variability is there }\DIFdelend \DIFaddbegin \DIFadd{the
variability }\DIFaddend in the latency\DIFdelbegin \DIFdel{?  }\DIFdelend \DIFaddbegin \DIFadd{.  }\DIFaddend We define jitter as the standard
...
%DIFDELCMD < %DIFDELCMD < \label{sec:fp-methods}%%%
%DIFDELCMD < 
...
promoter \mbox{%DIFAUXCMD
\cite{Markowitz:2015ko}}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Fibre photometry \mbox{%DIFAUXCMD
...
timing variability within each song and across songs (which \DIFdelbegin \DIFdel{may be responsible for some }\DIFdelend \DIFaddbegin \DIFadd{is
responsible for a small amount }\DIFaddend of our measured jitter\DIFaddbegin \DIFadd{, but note that few of the songs are significantly timeshifted}\DIFaddend ).
...
\begin{figure}
  ...
  \caption{Each plot shows one network output unit's responses to all
    ...
    $t^*_6$\DIFaddbeginFL \DIFaddFL{, and we do not show the non-response to presentation of non-song}\DIFaddendFL . The horizontal axis is time relative to the beginning of
...
%DIF < Accuracy \marginpar{Should be! Verify! And/or explain that there is a bug in the LabVIEW implementation, if there in fact is. ALSO need to present a picture of the test syllable or etc\dots} is identical across the three detectors.
%DIF > Accuracy \marginpar{Should be! Verify! And/or explain that there is a bug in the LabVIEW implementation, if there in fact is. ALSO need to present a picture of the test syllable or etc \dots} is identical across the three detectors.
\noprint{
...
single point in each chart in \fig{fig:accuracies}\DIFdelbegin \DIFdel{.  Some }\DIFdelend \DIFaddbegin \DIFadd{, and the means are
shown in Table~\ref{table:accuracies}.  Evidently some }\DIFaddend syllables are
easier to identify reliably than others \DIFdelbegin \DIFdel{, allowing detection point choice to impact the detector's accuracy.  For difficult syllables , }\DIFdelend \DIFaddbegin \DIFadd{(for example, $t^*_3$ and
$t^*_6$ are occasionally confused due to their similar appearance, as
could be guessed from }\fig{fig:song}\DIFadd{); thus, if the experimental
design permits, it can be worthwhile to choose an easily identifiable
syllable.  Furthermore, as can be seen from }\fig{fig:accuracies}\DIFadd{, for
difficult syllables }\DIFaddend the training process occasionally yields a network
that performs unusually poorly\DIFdelbegin \DIFdel{, but it
}\DIFdelend \DIFaddbegin \DIFadd{.  It }\DIFaddend is easy to spot these networks by
their performance either as measured as part of the training process
or on the test audio file, \DIFdelbegin \DIFdel{and
re-train when necessary}\DIFdelend \DIFaddbegin \DIFadd{perhaps taking the best of a few training
runs}\DIFaddend .
...
\begin{figure}
  ...
  \caption{Accuracy variability over 100 different training runs for
    ...
    by colour.  \DIFaddbeginFL \DIFaddFL{The means are given in Table~\ref{table:accuracies}.}\DIFaddendFL }
  \label{fig:accuracies}
\end{figure}

\DIFaddbegin \begin{table}
  \begin{tabular}{c|rr}
    \DIFaddFL{Target time }& \DIFaddFL{\% True positives }& \DIFaddFL{\% False positives }\\ \hline
    \DIFaddFL{$t^*_1$ }& \DIFaddFL{99.34 }&  \DIFaddFL{0.0015 }\\
    \DIFaddFL{$t^*_2$ }& \DIFaddFL{99.65 }&  \DIFaddFL{0.00040 }\\
    \DIFaddFL{$t^*_3$ }& \DIFaddFL{98.98 }&  \DIFaddFL{0.0015 }\\
    \DIFaddFL{$t^*_4$ }& \DIFaddFL{99.30 }&  \DIFaddFL{0.00093 }\\
    \DIFaddFL{$t^*_5$ }& \DIFaddFL{99.73 }&  \DIFaddFL{0.000094 }\\
    \DIFaddFL{$t^*_6$ }& \DIFaddFL{98.31 }&  \DIFaddFL{0.0026
  }\end{tabular}
  \caption{\DIFaddFL{Mean values for the detection accuracies shown in }\fig{fig:accuracies}\DIFaddFL{.}}
  \label{table:accuracies}
\end{table}


\DIFaddend We repeated the above experiment using only 2 hidden units per syllable.  Training under this condition is much faster, but the mean true positive rate per syllable decreases by an average of 18\% across our test syllables, and the mean false positive rate increases by roughly 23\%.
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{sec:fp-results}%%%
%DIFDELCMD < 

%DIFDELCMD < \fig{fig:fp-audio} %%%
...
sensory errors during singing \mbox{%DIFAUXCMD
\cite{MandelblatCerf:2014eu}}%DIFAUXCMD
.
}%DIFDELCMD < 
...
%DIFDELCMD <     \includegraphics[width=7cm]{Fig8}
...
\DIFdelFL{Audio amplitude stack plot of the experiment as aligned by
    ...
    of the trials.}}
  %DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:fp-audio}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}
%DIFDELCMD <   \begin{center}
%DIFDELCMD <     \includegraphics[width=7cm]{Fig9}
%DIFDELCMD <   \end{center}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Fibre photometry during AAF reveals an AAF-sensitive response
  ...
  %DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:fp-results}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
also likely to work well for Bengalese finches \DIFaddbegin \DIFadd{due to their similarly stereotyped syllables}\DIFaddend .  It offers the
following benefits:
 \begin{itemize} 
\item The detector is accurate. False negative and false positive
  rates can be well under \DIFdelbegin \DIFdel{0.5\% and 0.01}\DIFdelend \DIFaddbegin \DIFadd{1\% and 0.001}\DIFaddend \% respectively, and trading
...
detector is available under an open-source license {\em (\DIFaddbegin \DIFadd{Tentatively GPL, but }\DIFaddend we are still evaluating which open source license is the most suitable for this software):}
 \begin{description} 
\item[Training the neural network:]\hfill
  \begin{trivlist}
  \item {\tt https://github.com/\DIFdelbegin \DIFdel{bwpearre}\DIFdelend \DIFaddbegin \DIFadd{gardner-lab}\DIFaddend /\DIFdelbegin \DIFdel{birds/align}\DIFdelend \DIFaddbegin \DIFadd{syllable-detector-learn}\DIFaddend }
  \end{trivlist}

  \DIFaddbegin  \begin{description} 
  \item[Installation:]\hfill
     \begin{itemize} 
    \item \DIFadd{Clone the Github repository onto your computer.
    }\item \DIFadd{You also need the following packages from MATLAB File Exchange somewhere in your MATLAB path:
      } \begin{description} 
      \item[Significant Figures] \DIFadd{by Teck Por Lam.  File ID \#10669.
      }\item[Generate maximally perceptually-distinct colors] \DIFadd{by Tim Holy.  File ID \#29702.
      } \end{description} 
     \end{itemize} 
  \item[Usage:]\hfill
     \begin{itemize} 
      \item \DIFadd{The main programme is }{\tt \DIFadd{learn\_detector.m}}\DIFadd{.
    }\item \DIFadd{The top section of }{\tt \DIFadd{learn\_detector.m}} \DIFadd{contains global
      configuration variables.  The important ones are the data
      directory and the bird name: as supplied, the software looks for
      }{\tt \DIFadd{\textless data directory\textgreater/\textless bird
        name\textgreater/song.mat}}\DIFadd{.  Modify as necessary.
    }\item \DIFadd{The format for the data file }{\tt \DIFadd{\textless data
      directory\textgreater/\textless bird name\textgreater/song.mat}}
      \DIFadd{is specified in }{\tt \DIFadd{syllable-detector-learn/README.md}}\DIFadd{: a
      MATLAB file containing aligned song, nonsong (calls, cage noise,
      etc), and the audio sampling frequency.
    }\item \DIFadd{The configuration file }{\tt \DIFadd{\textless data
      directory\textgreater/\textless bird name\textgreater/params.m}}
      \DIFadd{is also described in }{\tt \DIFadd{README.md}}\DIFadd{.  If the file is empty, the
      ...
      syllables of interest, and exit.  If }{\tt \DIFadd{times\_of\_interest}}
      \DIFadd{is defined, the programme will train the neural network.
      Optionally, }{\tt \DIFadd{params.m}} \DIFadd{can also override any of the
      configuration parameters at the top of }{\tt \DIFadd{learn\_detector.m}}\DIFadd{.
    }\item \DIFadd{The software will produce a neural network file, }{\tt
      \DIFadd{detector\_bird\_times\_}\dots\DIFadd{.mat}} \DIFadd{(the ellipses signify other
      information encoded in the filename).  It will also produce the
      audio test file described in Methods.
    } \end{itemize} 
   \end{description} 

\DIFaddend \item[Runtime:]\hfill
   \begin{description} 
  \item[MATLAB:] {\tt https://github.com/gardner-lab/syllable-detector-matlab}
    ...
    \item[Installation:]\hfill
       \begin{itemize} 
      \item \DIFadd{Clone the Github repository onto your computer.
      }\item \DIFadd{Usage requires MATLAB with the Data Acquisition Toolbox and the 
      Parallel Computing Toolbox.
      } \end{itemize} 
    \item[Connection:]\hfill
       \begin{itemize} 
      \item \DIFadd{Either connect the microphone directly into the microphone jack or through a 
      preamp into the audio input jack. The current implementation only supports a single 
      channel of audio.
      }\item \DIFadd{To generate TTL signals via the audio output, use the headphone or audio output
      jack.
      }\item \DIFadd{To generate TTL signals via an Arduino, switch to the }{\tt \DIFadd{arduino}} \DIFadd{branch, 
    ...
    \item[Usage:]\hfill
       \begin{itemize} 
      \item \DIFadd{Switch to the directory of the repository and run }{\tt \DIFadd{nndetector\_live}}\DIFadd{. You
      ...
      }\item \DIFadd{Note that you may need to tune the input and output buffer based on computer 
      ...
      To specify the buffer sizes, use the }{\tt \DIFadd{buffer\_size\_input}} \DIFadd{and 
      }{\tt \DIFadd{buffer\_size\_output}} \DIFadd{parameters.
      }\item \DIFadd{Information about the detector is logged to a file called ``detector\_status.log''.
      } \end{itemize} 
     \end{description} 
  \DIFaddend \item[Swift:] {\tt https://github.com/gardner-lab/\DIFaddbegin \dots\DIFadd{\ }\DIFaddend syllable-detector-swift\DIFaddbegin \DIFadd{/releases/tag/0.3.1}\DIFaddend }
    \DIFaddbegin  \begin{description} 
    \item[Installation:]\hfill
      \begin{trivlist}
      \item \DIFadd{Download the most recent release from the Github repository onto your computer.
      }\end{trivlist}
    \item[Connection:]\hfill
       \begin{itemize} 
      \item \DIFadd{Either connect the microphone directly into the microphone jack or through a 
      ...
      }\item \DIFadd{To generate TTL signals via the audio output, use the headphone jack. The 
      audio output must have at least the same number of channels as the input.
      }\item \DIFadd{To generate TTL signals via an Arduino, load the MATLAB Arduino IO sketch 
    ...
    \item[Usage:]\hfill
       \begin{itemize} 
      \item \DIFadd{Use }{\tt \DIFadd{convert\_to\_text.m}} \DIFadd{to convert the Matlab output file to a format 
      easily readable by Swift. This will generate a text version of the neural network.
      }\item \DIFadd{Launch the application downloaded above. It will provide an 
    ...
    \item[Customisation:]\hfill
       \begin{itemize} 
      \item \DIFadd{You can clone the Github repository and use Xcode to edit the project, if you 
      ...
      }{\tt \DIFadd{Processor}} \DIFadd{class acts as a coordinator, receiving incoming audio, passing that
      to the detector and generating appropriate output TTL signals.
      } \end{itemize} 
     \end{description} 
  \DIFaddend \item[LabVIEW:] {\tt https://github.com/\DIFdelbegin \DIFdel{bwpearre}\DIFdelend \DIFaddbegin \DIFadd{gardner-lab}\DIFaddend /\DIFdelbegin \DIFdel{birds}\DIFdelend \DIFaddbegin \dots\DIFadd{\ syllable-detector-runtime-labview}}
     \begin{description} 
    \item[Installation:]\hfill
      \begin{trivlist}
      \item \DIFadd{Clone the Github repository onto your computer.
    ...
    \item[Connection:]\hfill
       \begin{itemize} 
      \item \DIFadd{The microphone should be connected, optionally through a preamplifier, to the data acquisition card, by default on Analog Input 0 (AI0).
      }\item \DIFadd{Outputs are on the Digital I}\DIFaddend /\DIFdelbegin \DIFdel{align}\DIFdelend \DIFaddbegin \DIFadd{O (DIO) channels.  See the block diagram of }{\tt \DIFadd{nnfft.vi}\DIFaddend } \DIFaddbegin \DIFadd{for documentation, or just connect to DIO0.
      } \end{itemize} 
    \item[Usage:]\hfill
       \begin{itemize} 
      \item \DIFadd{Use LabVIEW to open }{\tt \DIFadd{nnfft.vi}}\DIFadd{.
        }\item \DIFadd{A file chooser is provided in the front panel of }{\tt \DIFadd{nnfft.vi}}\DIFadd{.  Use it to point to the network file
      produced by }{\tt \DIFadd{learn\_detector.m}}\DIFadd{, and use LabView's ``Run''
...
\bibitem{Wang2008interhemispheric}
...
\bibitem{Charlesworth2011learning}
...
\bibitem{Noll1970pitchdetection}
Noll AM.

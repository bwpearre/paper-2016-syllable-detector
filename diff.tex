...
%DIF DEL ../paper-2016-syllable-detector-last-submitted-version/main.tex   Fri Mar 24 11:19:47 2017
%DIF ADD main.tex                                                          Fri Mar 24 10:53:22 2017
...
\begin{abstract}
  ...
  well under 1\% and \DIFdelbegin \DIFdel{0.001}\DIFdelend \DIFaddbegin \DIFadd{0.005}\DIFaddend \% respectively. The detector can run on a
...
second. The song may be repeated hundreds of times per day, \DIFaddbegin \DIFadd{and }\DIFaddend is almost
identical each time\DIFdelbegin \DIFdel{, and several }\DIFdelend \DIFaddbegin \DIFadd{.  Several }\DIFaddend brain areas reflect this consistency
in highly stereotyped neural firing patterns\DIFdelbegin \DIFdel{. This consistency }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend makes
the zebra finch one of the most popular models for the study of the
neural basis of learning, audition, and control.

\DIFdelbegin \DIFdel{This consistency allows a variety of experiments if }\DIFdelend \DIFaddbegin \DIFadd{If }\DIFaddend precise moments in song can reliably be detected quickly enough to
trigger other apparatus during singing\DIFaddbegin \DIFadd{, then this consistency of
behaviour allows a variety of experiments}\DIFaddend .  A common area of study
with \DIFdelbegin \DIFdel{this
song-triggering technique }\DIFdelend \DIFaddbegin \DIFadd{song-triggered experiments }\DIFaddend is the anterior forebrain pathway
...
song.  For example, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Kao2005} }%DIFAUXCMD
stimulated }\DIFdelend \DIFaddbegin \DIFadd{stimulation of }\DIFaddend the lateral magnocellular nucleus
of the anterior nidopallium (LMAN)---the output nucleus of the
AFP---at precisely timed moments during song \DIFdelbegin \DIFdel{and }\DIFdelend showed that this area
\DIFdelbegin \DIFdel{repeatably }\DIFdelend controls specific variables in song output \DIFdelbegin \DIFdel{.
\mbox{%DIFAUXCMD
\cite{Wang2008interhemispheric} }%DIFAUXCMD
stimulated }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Kao2005}}%DIFAUXCMD
.
Song-synchronised stimulation of }\DIFaddend LMAN and the high vocal centre (HVC)
in one hemisphere or the other \DIFdelbegin \DIFdel{, and found }\DIFdelend \DIFaddbegin \DIFadd{showed }\DIFaddend that control of song rapidly
switches between hemispheres \DIFdelbegin \DIFdel{.  \mbox{%DIFAUXCMD
\cite{Keller2009}
}%DIFAUXCMD
investigated the response of cells in field }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Wang2008interhemispheric}}%DIFAUXCMD
.
Feedback experiments have shown that Field }\DIFaddend L and the caudolateral
mesopallium \DIFdelbegin \DIFdel{during song with and without feedback perturbations as
well as external sounds, and concluded that these regions }\DIFdelend may hold a representation of song against which auditory
signals are compared \DIFdelbegin \DIFdel{.
\mbox{%DIFAUXCMD
\cite{Tumer2007pitchshift} }%DIFAUXCMD
played a disruptive sound during }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Keller2009}}%DIFAUXCMD
.  The disruption by white noise
of }\DIFaddend renditions of a syllable that were slightly above (or below) \DIFdelbegin \DIFdel{its average pitch ,
and }\DIFdelend \DIFaddbegin \DIFadd{the
syllable's average pitch }\DIFaddend showed that the \DIFdelbegin \DIFdel{presence of this }\DIFdelend apparently random natural
variability in songbird motor output is used to drive change in the
song \DIFdelbegin \DIFdel{.  \mbox{%DIFAUXCMD
\cite{Andalman2009} }%DIFAUXCMD
extended this experiment and showed that
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Tumer2007pitchshift}}%DIFAUXCMD
, and the }\DIFaddend AFP produces a
corrective signal to bias song away from those disruptions
\DIFdelbegin \DIFdel{.  \mbox{%DIFAUXCMD
\cite{Charlesworth2011learning} }%DIFAUXCMD
showed that the song change was }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Andalman2009}}%DIFAUXCMD
. The song change is }\DIFaddend isolated to within roughly 10
milliseconds (ms) of the stimulus, and \DIFdelbegin \DIFdel{further proposed an elegant model that predicts }\DIFdelend the shape of the learned
response \DIFdelbegin \DIFdel{and suggests }\DIFdelend \DIFaddbegin \DIFadd{can be predicted by }\DIFaddend a simple mechanism
\DIFdelbegin \DIFdel{.
\mbox{%DIFAUXCMD
\cite{Warren2011} }%DIFAUXCMD
showed that that }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Charlesworth2011learning}}%DIFAUXCMD
. The }\DIFaddend AFP transfers the error signal to the
robust nucleus of the arcopallium (RA) using NMDA-receptor--mediated
glutamatergic transmission \DIFdelbegin \DIFdel{. By looking at }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Warren2011}}%DIFAUXCMD
. The course of }\DIFaddend song
recovery after applying \DIFdelbegin \DIFdel{the same }\DIFdelend \DIFaddbegin \DIFadd{such a }\DIFaddend pitch-shift paradigm \DIFdelbegin \DIFdel{,
\mbox{%DIFAUXCMD
\cite{Canopoli2014} }%DIFAUXCMD
}\DIFdelend showed that the
caudal medial nidopallium is implicated in memorising or recalling a
recent song target, but \DIFdelbegin \DIFdel{for
}\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend neither auditory processing nor directed
motor learning \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Canopoli2014}}%DIFAUXCMD
}\DIFaddend .
...
is no standard syllable detector.  Desiderata \DIFaddbegin \DIFadd{for such a detector }\DIFaddend include:
 \begin{description} 
...
\item[Jitter:] The amount that latency changes from instance to instance of song.  Our measure of jitter is the standard deviation \DIFdelbegin \DIFdel{$\sigma$ }\DIFdelend of latency.
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Leonardo1999} }%DIFAUXCMD
used groups }\DIFdelend \DIFaddbegin \DIFadd{detection was achieved by a group }\DIFaddend of IIR filters with hand-tuned logical
operators \DIFdelbegin \DIFdel{.  Their }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Leonardo1999}}%DIFAUXCMD
.  The }\DIFaddend system had a latency of 50 or 100 ms,
and \DIFdelbegin \DIFdel{they did not report on jitter or accuracy }\DIFdelend \DIFaddbegin \DIFadd{accuracy and jitter were not reported}\DIFaddend .  As access to
computational resources has improved, approaches have changed: in
2009, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Andalman2009} }%DIFAUXCMD
still used }\DIFdelend hand-tuned filters \DIFdelbegin \DIFdel{, but ran them
}\DIFdelend \DIFaddbegin \DIFadd{were implemented }\DIFaddend on a Tucker-Davis Technologies digital signal processor\DIFdelbegin \DIFdel{.  They reported
a latency of }\DIFdelend \DIFaddbegin \DIFadd{, bringing latency down to }\DIFaddend around 4 ms \DIFdelbegin \DIFdel{, but }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Andalman2009}}%DIFAUXCMD
. But }\DIFaddend as with other filter-bank techniques, it
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Keller2009} }%DIFAUXCMD
applied }\DIFdelend \DIFaddbegin \DIFadd{2009 saw the application of }\DIFaddend a neural network to a spectral image of
song \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Keller2009}}%DIFAUXCMD
}\DIFaddend .  They reported a jitter of 4.3 ms, but further implementation and
performance details are not available.  In 2011, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Warren2011}
}%DIFAUXCMD
matched spectral templates to }\DIFdelend stable portions of syllables \DIFaddbegin \DIFadd{were matched to spectral templates }\DIFaddend in 8-ms
segments \DIFdelbegin \DIFdel{.  They reported }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Warren2011}}%DIFAUXCMD
.  This detector achieved }\DIFaddend a jitter of 4.5 ms, and false-negative and
false-positive rates of up to 2\% and 4\% \DIFdelbegin \DIFdel{, }\DIFdelend respectively.  Hardware
requirements and ease of use were not reported.  In 2013, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Skocik2013} }%DIFAUXCMD
described in detail a system that matches }\DIFdelend spectral
images of template syllables \DIFaddbegin \DIFadd{were compared to song }\DIFaddend using a correlation coefficient \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Skocik2013}}%DIFAUXCMD
}\DIFaddend .  With a
...
other work, \DIFdelbegin \DIFdel{an allusion is made to }\DIFdelend a syllable detector \DIFdelbegin \DIFdel{, but no further
information is provided}\DIFdelend \DIFaddbegin \DIFadd{is alluded to, but not described}\DIFaddend .
...
\item Record and align a corpus of training songs.  The technique has
  been published in \cite{Poole2012}. \DIFaddbegin \DIFadd{As few as 200 songs can yield
  acceptable results, but here we standardise on 1000-song training
  sets.
}\DIFaddend \item Choose one or more instants in the song that should create a trigger event, and train a
  neural network to recognise them. This step is carried out offline.  While any neural network software would produce \DIFdelbegin \DIFdel{equivalent }\DIFdelend \DIFaddbegin \DIFadd{similar }\DIFaddend results, we used MATLAB 2015b's neural network toolbox.
\item Once trained and saved, the neural network is used by a realtime
  detection \DIFdelbegin \DIFdel{program }\DIFdelend \DIFaddbegin \DIFadd{programme }\DIFaddend that listens to an audio signal and indicates detection of the target
  syllables via a TTL pulse.  We present three \DIFaddbegin \DIFadd{detection }\DIFaddend implementations, in MATLAB, LabVIEW, and Swift, that trade off hardware requirements, ease of maintenance, and performance.
 \end{enumerate} 
This method makes the following contributions:
 \begin{itemize} 
\item Fast: sub-millisecond latencies, with jitter around 2 ms.
\item Accurate: false negative rates under 1\% and false positive rates under \DIFdelbegin \DIFdel{0.001}\DIFdelend \DIFaddbegin \DIFadd{0.005}\DIFaddend \% for a variety of syllables.
\DIFdelbegin \DIFdel{Balancing these rates depends on a single relative-cost parameter.
}\DIFdelend \item State-of-the-art performance with default parameters.
\item \DIFaddbegin \DIFadd{Requires almost no programming experience.
}\item \DIFaddend Runs on inexpensive hardware.
...
\begin{figure}
  % Width: 8, Height: 3, Fonts scale: 120%
  \includegraphics[width=\textwidth]{Fig1}
  \caption{The spectrogram of \DIFdelbeginFL \DIFdelFL{a typical }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL song \DIFaddbeginFL \DIFaddFL{of the bird ``lny64''}\DIFaddendFL , used as \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL example
...
$\tau(t-n_\textrm{fft}t_\textrm{fft})\}$.  \DIFdelbegin \DIFdel{We have found that a 30-ms
time window---which will yield $n_\textrm{fft} = |T|=\lfloor 30\textrm{
  ms}/t_\textrm{fft}\rceil$ }\DIFdelend \DIFaddbegin \DIFadd{Time windows of 30--50
ms---the latter will yield $n_\textrm{fft} = |T|=\lfloor 50\textrm{
  ms}/t_\textrm{fft}\rceil$ }\DIFaddend frames---of frequencies spanning 1--8 kHz
\DIFdelbegin \DIFdel{works wellfor most syllables}\DIFdelend \DIFaddbegin \DIFadd{generally work well}\DIFaddend .

Six examples of chosen target moments in the song, \DIFdelbegin \DIFdel{and their
corresponding recognition
regions $F$ and $T$}\DIFdelend \DIFaddbegin \DIFadd{with recognition
regions $F =$ 1--8 kHz and $T=30$ ms}\DIFaddend , are shown in \fig{fig:song}.
...
network \DIFaddbegin \DIFadd{and to maximise the effectiveness of the neural network's training
algorithm}\DIFaddend , we normalise the incoming data stream so that changes in the
song structure of the sound are emphasised over changes in volume\DIFdelbegin \DIFdel{, and
...
jitter, but also increases training time. \DIFaddbegin \DIFadd{Sometimes the effects are
...
interface included in Apple's \DIFaddbegin \DIFadd{Mac OS X }\DIFaddend operating systems.
...
processor at 3.7GHz (a relatively low-end machine) with \DIFdelbegin \DIFdel{32 }\DIFdelend \DIFaddbegin \DIFadd{24 }\DIFaddend gigabytes
of RAM, running Microsoft Windows 8.1 Pro \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{and Windows 10.
}\DIFaddend 

This implementation has several drawbacks: it requires \DIFdelbegin \DIFdel{Windows and
}\DIFdelend expensive hardware and software \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{from National Instruments (a data acquisition card and LabVIEW), Windows (we use hardware features of LabVIEW that are unavailable on MacOS or Linux), and }\DIFaddend due to the programming language it
...
test configuration \DIFdelbegin \DIFdel{usually achieved }\DIFdelend \DIFaddbegin \DIFadd{proved itself capable of }\DIFaddend excellent performance, and further
...
While it did not perform as well \DIFdelbegin \DIFdel{on our test system }\DIFdelend as the other
...
interface, reducing latency by \DIFaddbegin \DIFadd{close to }\DIFaddend half.

\subsection{Quantification}
\label{sec:quantify}

We \DIFdelbegin \DIFdel{have used this detector for song alignment and feedback experiments
in 5 behaving birds, and for offline analysis on a few others.  For
simplicity, and to test robustness to imperfect choice of ``syllable'', we present performance on }\DIFdelend \DIFaddbegin \DIFadd{measure accuracy and characterise timing for }\DIFaddend 6 randomly chosen
points in one bird's song \DIFaddbegin \DIFadd{(lny64), and present accuracy results for
seven more birds (the detector's timing does not appreciably vary across
birds)}\DIFaddend . The results presented here are representative of real-world
performance \DIFdelbegin \DIFdel{over }\DIFdelend \DIFaddbegin \DIFadd{in feedback experiments on }\DIFaddend many birds in our lab.


\DIFdelbegin \DIFdel{We divided the data into training and test sets.  For the results
reported in the following section, the training set consisted of }\DIFdelend \DIFaddbegin \begin{table}
 \begin{tabular}{c|ccc}
    \DIFaddFL{Bird }& \DIFaddFL{Test songs = Test non-songs }& \DIFaddFL{Target time (ms) }\\ \hline
    \DIFaddFL{lny64 }& \DIFaddFL{1818 }& \DIFaddFL{150:50:400 }\\
    \DIFaddFL{lny46 }& \DIFaddendFL 1000 \DIFdelbeginFL \DIFdelFL{instances of the song of one bird, and }\DIFdelendFL \DIFaddbeginFL & \DIFaddFL{195 }\\
    \DIFaddFL{lny42 }& \DIFaddFL{1245 }& \DIFaddFL{380 }\\
    \DIFaddFL{lny4rb }& \DIFaddFL{1000 }& \DIFaddFL{250 }\\
    \DIFaddFL{lr12 }& \DIFaddendFL 2000 \DIFaddbeginFL & \DIFaddFL{180 }\\
    \DIFaddFL{lr13 }& \DIFaddFL{2000 }& \DIFaddFL{300 }\\
    \DIFaddFL{lr28 }& \DIFaddFL{3000 }& \DIFaddFL{140 }\\
    \DIFaddFL{lr77 }& \DIFaddFL{2000 }& \DIFaddFL{320
  }\end{tabular}
  \caption{\DIFaddFL{For each bird, detectors were trained for the specified
...
\DIFadd{Data consisted of extracted, aligned songs, and additional }\DIFaddend song-length
samples of non-song (cage noise, calls, other birds' song). \DIFdelbegin \DIFdel{Our test }\DIFdelend \DIFaddbegin \DIFadd{These were
divided into training and test sets. For all birds, we used 1000 songs
and 1000 non-song samples for training. The test }\DIFaddend set consisted of \DIFdelbegin \DIFdel{an additional 1818 iterations of the bird's song and 3636
song-length samples of non-song}\DIFdelend \DIFaddbegin \DIFadd{the remaining songs and an equal number of non-songs. Table~\ref{table:datasizes}
...
other birds, we eyeballed one promising-looking detection point}\DIFaddend .
...
Because the data set consists of temporally aligned songs, \DIFdelbegin \DIFdel{ground
truth is available for each song }\DIFdelend \DIFaddbegin \DIFadd{the moment
at which detection should occur during each song sample is available
}\DIFaddend (albeit with minor variations due to the alignment process used
\cite{Poole2012})\DIFdelbegin \DIFdel{, and thus the }\DIFdelend \DIFaddbegin \DIFadd{.  The }\DIFaddend detector can be checked by \DIFdelbegin \DIFdel{presenting }\DIFdelend \DIFaddbegin \DIFadd{comparing its
response to }\DIFaddend the recorded training songs \DIFdelbegin \DIFdel{as well as
the canonical}\DIFdelend \DIFaddbegin \DIFadd{against these ``canonical''
}\DIFaddend detection events.  To this end, besides the trained network object,
...
pulses may be compared against the \DIFaddbegin \DIFadd{``ground truth'' given by the
canonical detection event provided by the }\DIFaddend right channel.
...
non-detected \DIFdelbegin \DIFdel{non-syllable }\DIFdelend \DIFaddbegin \DIFadd{non-matching frame }\DIFaddend counting as a true negative, we also
...
definition of \DIFaddbegin \DIFadd{the }\DIFaddend false-positive rate for data streams that had not been
...
\DIFdelbegin \DIFdel{Mean latency is then given as the average }\DIFdelend \DIFaddbegin \DIFadd{We define latency as the }\DIFaddend time between the \DIFdelbegin \DIFdel{canonical
signal }\DIFdelend \DIFaddbegin \DIFadd{training target (during our tests, this is indicated by the canonical
signal in the test file) }\DIFaddend and the corresponding detection event.  It is a helpful number,
but not a critical one\DIFdelbegin \DIFdel{, since in zebra finch}\DIFdelend \DIFaddbegin \DIFadd{; due to the stereotyped song of the zebra finch, }\DIFaddend a detector with high but
...
occur at the very beginning of a syllable, the detector \DIFdelbegin \DIFdel{'s time window
may include previous syllable ).  Often more important is }\DIFdelend \DIFaddbegin \DIFadd{may be trained 
to recognise the previous syllable and the gap thereafter).  Usually }\DIFaddend the 
variability in \DIFdelbegin \DIFdel{the latency }\DIFdelend \DIFaddbegin \DIFadd{latency is the more important number}\DIFaddend .  We define jitter as 
the standard deviation of \DIFdelbegin \DIFdel{the latency}\DIFdelend \DIFaddbegin \DIFadd{detection latency, measured over the test songs}\DIFaddend .
...
\fig{fig:song} shows \DIFdelbegin \DIFdel{the songfor which we present accuracy and timing
results.  Six example }\DIFdelend \DIFaddbegin \DIFadd{a typical song. The image is produced by
averaging the aligned spectrograms for the bird lny64.  Six }\DIFaddend target
trigger points are shown \DIFaddbegin \DIFadd{for this song}\DIFaddend , at 150, 200, 250, 300, 350,
and 400 milliseconds after the beginning of the aligned samples, as
indicated by the red lines\DIFdelbegin \DIFdel{, and referred to henceforth }\DIFdelend \DIFaddbegin \DIFadd{. In discussion, we will refer to these six
}\DIFaddend as $t^*_1\ldots t^*_6$ respectively.  \DIFdelbegin \DIFdel{The rectangles show
the recognition window (}\DIFdelend \DIFaddbegin \DIFadd{To avoid overlap in the graphic, the rectangles show
a shorter recognition window than our standard: here }\DIFaddend 30 ms, \DIFaddbegin \DIFadd{but still
spanning }\DIFaddend 1--8 kHz\DIFdelbegin \DIFdel{) for each instant }\DIFdelend \DIFaddbegin \DIFadd{, for each target time }\DIFaddend in the song.

The first two triggers, $t^*_1$ at 150 ms and $t^*_2$ at 200 ms, are
far from pure tones, but are closer to \DIFdelbegin \DIFdel{coloured }\DIFdelend \DIFaddbegin \DIFadd{bandpass-filtered white }\DIFaddend noise.  They would be
...
responsible for a small amount of our measured jitter, but note that few of the songs are significantly \DIFdelbegin \DIFdel{timeshifted}\DIFdelend \DIFaddbegin \DIFadd{time-shifted}\DIFaddend ).
...
Accuracies \DIFdelbegin \DIFdel{for our six test syllables }\DIFdelend are shown in \fig{fig:accuracies} (accuracies for the synthetic $\delta$-function
songs were always 100\%).  For each test syllable we trained 100
different networks, which differ from each other \DIFaddbegin \DIFadd{both }\DIFaddend in the random
initialisation of network weights and \DIFdelbegin \DIFdel{on which }\DIFdelend \DIFaddbegin \DIFadd{in the }\DIFaddend random subset of \DIFdelbegin \DIFdel{our
songs }\DIFdelend \DIFaddbegin \DIFadd{the
song-and-nonsong corpus that }\DIFaddend was used for training.  Each network's
...
identify reliably than others (for example, \DIFaddbegin \DIFadd{for lny64, }\DIFaddend $t^*_3$ \DIFaddbegin \DIFadd{(250 ms) }\DIFaddend and $t^*_6$ \DIFaddbegin \DIFadd{(400 ms) }\DIFaddend are
...
\begin{figure}
  ...
  \caption{Accuracy variability over 100 different training runs for
    each of the \DIFdelbeginFL \DIFdelFL{six }\DIFdelendFL test \DIFdelbeginFL \DIFdelFL{syllables}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{detection points}\DIFaddendFL .  Each dot shows the test-set
...
\begin{table}
  \DIFdelbeginFL %DIFDELCMD < \begin{tabular}{c|rr}
%DIFDELCMD <     %%%
\DIFdelFL{Target time }\DIFdelendFL %DIF >  replot_accuracies_concatanated('tex', true) for confusion_log_perf.txt (1.5 ms).
  \DIFaddbeginFL \begin{tabular}{lc|cc}
    \DIFaddFL{Bird }& \DIFaddFL{Target time (ms) }\DIFaddendFL & \% True positives & \% False positives \\ \hline
\DIFdelbeginFL \DIFdelFL{$t^*_1$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lny64 }\DIFaddendFL & 	\DIFdelbeginFL \DIFdelFL{99.34 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{150 $[t^*_1]$ }\DIFaddendFL & 	\DIFdelbeginFL \DIFdelFL{0.0015 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{99.66 }& 	\DIFaddFL{0.0026 }\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$t^*_2$ }\DIFdelendFL %DIF >  /Volumes/Data/song/lny64/confusion_log_perf.txt
      & 	\DIFdelbeginFL \DIFdelFL{99.65 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{200 $[t^*_2]$ }\DIFaddendFL & 	\DIFdelbeginFL \DIFdelFL{0.00040 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{99.75 }& 	\DIFaddFL{0.00052 }\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$t^*_3$ }\DIFdelendFL %DIF >                                                
      & 	\DIFdelbeginFL \DIFdelFL{98.98 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{250 $[t^*_3]$ }\DIFaddendFL & 	\DIFdelbeginFL \DIFdelFL{0.0015 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{99.79 }& 	\DIFaddFL{0.00051 }\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$t^*_4$ }\DIFdelendFL %DIF >                                                
      & 	\DIFdelbeginFL \DIFdelFL{99.30 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{300 $[t^*_4]$ }\DIFaddendFL & 	\DIFdelbeginFL \DIFdelFL{0.00093 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{99.70 }& 	\DIFaddFL{0.00077 }\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$t^*_5$ }\DIFdelendFL %DIF >                                                
      & 	\DIFaddbeginFL \DIFaddFL{350 $[t^*_5]$ }& 	\DIFaddendFL 99.73 & 	\DIFdelbeginFL \DIFdelFL{0.000094 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.00022 }\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$t^*_6$ }\DIFdelendFL %DIF >                                                
      & 	\DIFdelbeginFL \DIFdelFL{98.31 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{400 $[t^*_6]$ }\DIFaddendFL & 	\DIFaddbeginFL \DIFaddFL{99.52 }& 	\DIFaddFL{0.0010 }\\ %DIF >                                                 
\DIFaddFL{lny42 }& 	\DIFaddFL{380 }& 	\DIFaddFL{99.09 }& 	\DIFaddFL{0.0011 }\\ %DIF >  /Volumes/Data/song/lny42/confusion_log_perf.txt          
\DIFaddFL{lny46 }& 	\DIFaddFL{140 }& 	\DIFaddFL{99.11 }& 	\DIFaddFL{0.0036 }\\ %DIF >  /Volumes/Data/song/lny46/confusion_log_perf.txt          
\DIFaddFL{lny4rb }& 	\DIFaddFL{250 }& 	\DIFaddFL{99.37 }& 	\DIFaddendFL 0.0026 \DIFaddbeginFL \\ %DIF >  /Volumes/Data/song/lny4rb/confusion_log_perf.txt        
\DIFaddFL{lr12 }& 	\DIFaddFL{180 }& 	\DIFaddFL{99.52 }& 	\DIFaddFL{0.0033 }\\ %DIF >  /Volumes/Data/song/lr12/all/confusion_log_perf.txt        
\DIFaddFL{lr13 }& 	\DIFaddFL{300 }& 	\DIFaddFL{99.83 }& 	\DIFaddFL{0.0013 }\\ %DIF >  /Volumes/Data/song/lr13/all/confusion_log_perf.txt        
\DIFaddFL{lr28 }& 	\DIFaddFL{140 }& 	\DIFaddFL{99.73 }& 	\DIFaddFL{0.0028 }\\ %DIF >  /Volumes/Data/song/lr28/all/confusion_log_perf.txt        
\DIFaddFL{lr77 }& 	\DIFaddFL{320 }& 	\DIFaddFL{99.58 }& 	\DIFaddFL{0.0028  %DIF >  /Volumes/Data/song/lr77/all/confusion_log_perf.txt          
...
\DIFaddbegin \DIFadd{Changing the FFT interval to 2 ms significantly reduces the
...
of syllables \DIFdelbegin \DIFdel{chosen }\DIFdelend \DIFaddbegin \DIFadd{shown }\DIFaddend here, performance was \DIFdelbegin \DIFdel{fairly }\DIFdelend stable across syllables.
\fig{fig:TimingVsSyllable} shows measured timing data for \DIFaddbegin \DIFadd{lny64's test
syllables for }\DIFaddend the Swift+serial detector compared to the ideal, with
...
\begin{figure}
  ...
  \end{center}
  \caption{Timing data for \DIFdelbeginFL \DIFdelFL{our }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lny64's }\DIFaddendFL 6 test syllables, for the ideal and the
...
\vspace{8pt}
\DIFdelbegin %DIFDELCMD < \begin{tabular}{l|c|c|c| c}
%DIFDELCMD <   %%%
...
The \DIFdelbegin \DIFdel{song presented here was recorded with a fixed microphone }\DIFdelend \DIFaddbegin \DIFadd{songs presented here were recorded with fixed microphones }\DIFaddend mounted
inside the \DIFdelbegin \DIFdel{cage}\DIFdelend \DIFaddbegin \DIFadd{cages}\DIFaddend .  We found that higher accuracy is achieved when a
...
\DIFdelbegin %DIFDELCMD < {\em %%%
\DIFdel{If accepted, before final submission, DOIs for the latest code will be created as described in https: }\DIFdelend \DIFaddbegin  \begin{description} 
\item[Data:]
  \DIFadd{Data and training configuration files for the 8 birds discussed here are available from the Open Science Framework at DOI: 10.17605}\DIFaddend /\DIFaddbegin \DIFadd{OSF.IO}\DIFaddend /\DIFdelbegin \DIFdel{guides.github.com/activities/citable-code/ so that our results may be reproduced and the software deployed. For now, in lieu of final release DOIs , we provide these links to our development repositories.  If you would like final versions and test datasets for review, please contact the corresponding author (bwpearre@bu.
edu).}%DIFDELCMD < }
%DIFDELCMD < %%%
%DIF < \marginpar{PLOS doesn't really say where such things should go.}
%DIF < \marginpar{Cleanup: mostly done.  But it would be nice to move all this to the lab's GitHub group.}
%DIFDELCMD < \begin{description}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \DIFadd{BX76R . DOIs for the versions of the programmes used here are provided for reproducibility; however, we generally recommend downloading the latest release.
%DIF > \item[Licenses:]
%DIF >   All software is licensed under Open-Source-Initiative--approved licenses. Please see the software packages for individual choices.
}\DIFaddend \item[Song alignment:] Jeff Markowitz's song alignment software \cite{Poole2012} can be found at
    {\tt https://github.com/jmarkow/zftftb}
\DIFdelbegin %DIFDELCMD < \end{description}
%DIFDELCMD < 

%DIFDELCMD < \noindent %%%
\DIFdel{Our implementation of the syllable
detector is available under an open-source license }%DIFDELCMD < {\em %%%
\DIFdel{(Tentatively GPL, but we are still evaluating which open source license is the most suitable for this software):}%DIFDELCMD < }
%DIFDELCMD < \begin{description}
%DIFDELCMD < %%%
\DIFdelend \item[Training the neural network:]\hfill
  \begin{trivlist}
  \item {\tt https://github.com/gardner-lab/syllable-detector-learn}
    \DIFaddbegin \item \DIFadd{DOI: 10.5281/zenodo.437555
  }\DIFaddend \end{trivlist}

   \begin{description} 
  \item[Installation:]\hfill\DIFaddbegin \\
    \DIFadd{See }{\tt \DIFadd{README.md}} \DIFadd{in the repository for complete, up-to-date instructions.
    }\DIFaddend  \begin{itemize} 
    \item Clone the Github repository onto your computer\DIFaddbegin \DIFadd{, and add it to your MATLAB path}\DIFaddend .
    \item You also need the following packages from MATLAB File Exchange \DIFdelbegin \DIFdel{somewhere }\DIFdelend in your MATLAB path:
       \begin{description} 
  ...
  \item[Usage:]\hfill
    ...
    \item The top section of {\tt learn\_detector.m}
      \DIFdelbegin \DIFdel{contains global
      }\DIFdelend \DIFaddbegin \DIFadd{describes the }\DIFaddend configuration variables.
    \DIFdelbegin \DIFdel{The important ones are the data
      directory and the bird name: as supplied, the software looks for
      }\DIFdelend \DIFaddbegin \item \DIFadd{The current directory should contain the data in }\DIFaddend {\tt
      \DIFdelbegin \DIFdel{\textless data directory\textgreater/\textless bird
        name\textgreater/}\DIFdelend song.mat}. \DIFdelbegin \DIFdel{Modify as necessary.
    }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend The format for \DIFdelbegin \DIFdel{the data file }%DIFDELCMD < {\tt %%%
\DIFdel{\textless data
      directory\textgreater/\textless bird name\textgreater/song.mat}%DIFDELCMD < }
%DIFDELCMD <       %%%
\DIFdelend \DIFaddbegin \DIFadd{this file }\DIFaddend is specified in {\tt
      \DIFdelbegin \DIFdel{syllable-detector-learn/}\DIFdelend README.md}: a MATLAB file containing aligned song, nonsong
      (calls, cage noise, etc), and the audio sampling frequency.
    \item \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin {\tt \DIFadd{learn\_detector}} \DIFadd{also looks in the current directory for
      a }\DIFaddend configuration file {\tt \DIFdelbegin \DIFdel{\textless data
      directory\textgreater/\textless bird name\textgreater/}\DIFdelend params.m}\DIFdelbegin \DIFdel{is also }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend described in {\tt
      ...
      and exit.  If {\tt times\_of\_interest\DIFaddbegin \DIFadd{\_ms}\DIFaddend } is defined, the
    ...
    \item \DIFaddbegin \DIFadd{Configuration may also be controlled by specifying
      parameter-value pairs to the function call in the common MATLAB
      fashion; for example, }{\tt
        \DIFadd{learn\_detector('times\_of\_interest\_ms', }[\DIFadd{150 200}]\DIFadd{)}}\DIFadd{.
    }\item \DIFaddend The software will produce a neural network file, {\tt
...
\item[Runtime:]\hfill
  ...
  \DIFaddend \item[MATLAB:]\DIFaddbegin \hfill
    \begin{trivlist}
    \item \DIFaddend {\tt https://github.com/gardner-lab/syllable-detector-matlab}
    \DIFaddbegin \item \DIFadd{DOI: 10.5281/zenodo.437557
    }\end{trivlist}
  ...
  \DIFaddend \item[Swift:]\DIFaddbegin \hfill
    \begin{trivlist}
    \item \DIFaddend {\tt https://github.com/gardner-lab/\DIFdelbegin %DIFDELCMD < \dots%%%
\DIFdel{\ }\DIFdelend syllable-detector-swift\DIFdelbegin \DIFdel{/releases/tag/0.3.1}\DIFdelend }
    \DIFaddbegin \item \DIFadd{DOI: 10.5281/zenodo.437559
    }\end{trivlist}
    \DIFaddend  \begin{description} 
    \item[Installation:]\hfill
      \begin{trivlist}
...
%DIF < \bibliography{birds}
\DIFdelbegin %DIFDELCMD < \begin{thebibliography}{10}
%DIFDELCMD < 

%DIFDELCMD < \bibitem{Kao2005}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.nature.com/nature/journal/v433/n7026/abs/nature03127.html}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Wang2008interhemispheric}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.0060250}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Keller2009}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.nature.com/nature/journal/v457/n7226/abs/nature07467.html}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Tumer2007pitchshift}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.nature.com/nature/journal/v450/n7173/abs/nature06390.html}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Andalman2009}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.ncbi.nlm.nih.gov/pmc/articles/PMC2709669/}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Charlesworth2011learning}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3045469/}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Warren2011}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://jn.physiology.org/content/106/4/1806.full}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Canopoli2014}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.jneurosci.org/content/34/20/7018.full}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Leonardo1999}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.nature.com/nature/journal/v399/n6735/abs/399466a0.html}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Skocik2013}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3539774/}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Poole2012}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://journals.plos.org/plosone/article?id=10.1371/journal.pone.0038173}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Tchernichovski2011SAP}
%DIFDELCMD < %%%
\DIFdel{Tchernichovski O. Sound Analysis Pro User Manual; 2011.
}%DIFDELCMD < \newblock %%%
\DIFdel{Available from: }%DIFDELCMD < \url{http://soundanalysispro.com/manual-1}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{Jacobsen2003SlidingDFT}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.cmlab.csie.ntu.edu.tw/DSPCourse/reference/Sliding\%20DFT.pdf}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{hkp}
%DIFDELCMD < %%%
...
%DIFDELCMD < \bibitem{Noll1970pitchdetection}
%DIFDELCMD < %%%
\DIFdel{Noll AM.
}%DIFDELCMD < \newblock %%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{birds}
\DIFaddend 

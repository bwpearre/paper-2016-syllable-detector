...
brain areas concerned with the learning and production of song.  For
...
\item Accurate: false negative rates under 1\% and false positive rates under 0.001\% for a variety of syllables.  Balancing these rates depends on a single relative-cost parameter.
...
Because the data set consists of temporally aligned songs, the moment at which detection should occur during each song sample is available (albeit with minor variations due to
the alignment process used \cite{Poole2012}).  The detector
can be checked by comparing its response to the recorded training song samples against
these ``canonical'' detection events.  To this end, besides the trained

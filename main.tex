...
We begin with a few hundred recordings of a given bird's song.  Male
...
more challenging.  The songs are recorded inside a sound-isolating
chamber in which was mounted a recording microphone (Audio-Technica
AT803).  The songs are time-aligned as described in \cite{Poole2012}.
...
with as low as a 1:1 ratio of non-song to song yields excellent
results on most birds.  These data are also included in our accuracy tests.
...
We used the Levenberg-Marquardt algorithm, which is the MATLAB
...
We evaluate the time taken from the presentation of each instance of the target
syllable to the firing of the detector's TTL pulse---for example, if the target trigger point with respect to the aligned songs is at 200 ms, and for a given presentation of the song the detector fires at 203 ms, we take the latency on that trial to be 3 ms.

While playing the
audio test file from any audio playback device, we used the TTL output from
the ground-truth channel of the audio output as the
trigger pulse for an oscilloscope, and compared it to the TTL pulse
produced by the detector, which sees only the birdsong
...
It is a helpful number, but not a critical one, since in zebra finch a
detector with high but constant latency can often be trained to
trigger at a point somewhat before the true moment of interest (for example, if triggering should occur at the very beginning of a syllable, the detector's time window may include previous syllable).  Often more
important is the variability in the
...
All procedures were approved by the Institutional Animal Care and Use Committee of Boston University (protocol number 14-029).

\section{Results}
\label{sec:results}

We have used this detector for song alignment and feedback experiments
in 5 behaving birds, and for offline analysis on various others.  For
simplicity we present performance on 6 randomly chosen points in one
bird's song, but the results presented here are representative of
real-world performance in our lab.\marginpar{Ok?}
...
detector is available under an open-source license {\em (we are still evaluating which open source license is the most suitable for this software):}
\begin{description}
\item[Training the neural network:]\hfill
  ...
  \item[Usage:]\hfill
    ...
    \item The top section of {\tt learn\_detector.m} contains global configuration variables.  The important ones are the data directory and the bird name: as supplied, the software looks for {\tt \textless data directory\textgreater/\textless bird name\textgreater/song.mat}.  Modify as necessary.
    \item The format for the data file {\tt \textless data directory\textgreater/\textless bird name\textgreater/song.mat} is specified in {\tt birds/align/README.md}: a MATLAB file containing aligned song, nonsong (calls, cage noise, etc), and the sampling frequency.
    \item The configuration file {\tt \textless data directory\textgreater/\textless bird name\textgreater/params.m} is also described in {\tt README.md}.  If the file is empty, the programme will display an image showing the average of the song spectrograms (frequency vs.~time), tell the user how to define syllables of interest, and exit.  If {\tt times\_of\_interest} is defined, the programme will train the neural network.  {\tt params.m} can also override any of the configuration parameters at the top of {\tt learn\_detector.m}.
    \item The software will produce a neural network file, {\tt detector\_bird\_times\_\dots.mat} (the ellipses signify other information encoded in the filename).  It will also produce the audio test file described in Methods.
    \end{itemize}
  \end{description}
...
\item[Runtime:]\hfill
  ...
  \item[LabVIEW:] {\tt https://github.com/bwpearre/birds/align}
    ...
    \item[Connection:]\hfill
      \begin{itemize}
      \item The microphone should be connected through a preamplifier to the data acquisition card, by default on Analog Input 0 (AI0).
      ...
      \end{itemize}
    \item[Usage:]\hfill
      Use LabVIEW to open {\tt nnfft.vi}.  A file
      chooser is provided in the front panel.  Use it to point to the network file
...
\bibitem{Markowitz:2015ko}
Markowitz JE, Liberti WA, Guitchounts G, Velho T, Lois C, Gardner TJ.
\newblock Mesoscopic Patterns of Neural Activity Support Songbird Cortical
  Sequences.
\newblock PLOS Biology. 2015 June;13(6):e1002158--20.
\newblock Available from:
  \url{http://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.1002158}.

\bibitem{Adelsberger:2005dy}
Adelsberger H, Garaschuk O, Konnerth A.
\newblock {Cortical calcium waves in resting newborn mice.}
\newblock Nature Neuroscience. 2005 August;8(8):988--990.
\newblock Available from:
  \url{http://www.nature.com/neuro/journal/v8/n8/full/nn1502.html}.

\bibitem{Schulz:2012fv}
Schulz K, Sydekum E, Krueppel R, Engelbrecht CJ, Schlegel F, Schr{\"o}ter A,
  et~al.
\newblock {Simultaneous BOLD fMRI and fiber-optic calcium recording in rat
  neocortex}.
\newblock Nature Methods. 2012 May;9(6):597--602.
\newblock Available from:
  \url{http://www.nature.com/nmeth/journal/v9/n6/full/nmeth.2013.html}.

\bibitem{Cui:2013dq}
Cui G, Jun SB, Jin X, Pham MD, Vogel SS, Lovinger DM, et~al.
\newblock {Concurrent activation of striatal direct and indirect pathways
  during action initiation}.
\newblock Nature. 2013 February;494(7436):238--242.
\newblock Available from:
  \url{http://www.nature.com/nature/journal/v494/n7436/full/nature11846.html}.

\bibitem{Adelsberger:2014jd}
Adelsberger H, Zainos A, Alvarez M, Romo R, Konnerth A.
\newblock {Local domains of motor cortical activity revealed by fiber-optic
  calcium recordings in behaving nonhuman primates.}
\newblock Proceedings of the National Academy of Sciences of the United States
  of America. 2014 January;111(1):463--468.
\newblock Available from: \url{http://www.pnas.org/content/111/1/463.full}.

\bibitem{Gunaydin:2014dh}
Gunaydin LA, Grosenick L, Finkelstein JC, Kauvar IV, Fenno LE, Adhikari A,
  et~al.
\newblock {Natural neural projection dynamics underlying social behavior.}
\newblock Cell. 2014 June;157(7):1535--1551.
\newblock Available from:
  \url{http://www.cell.com/cell/fulltext/S0092-8674%2814%2900659-X}.

\bibitem{MandelblatCerf:2014eu}
Mandelblat-Cerf Y, Las L, Denissenko N, Fee M.
\newblock {A role for descending auditory cortical projections in songbird
  vocal learning}.
\newblock Elife. 2014 January;3(0):e02152--e02152.
\newblock Available from: \url{https://elifesciences.org/content/3/e02152}.

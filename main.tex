...
own testing on a distinct and much larger data set, we instructed the
toolbox to use 80\% of its training set for training, and the
...
We evaluate the time taken from the presentation of each instance of
the target syllable to the firing of the detector's TTL pulse---for
example, if the target trigger point with respect to the aligned songs
is at 200 ms, and for a given presentation of the song the detector
fires at 203 ms, we take the latency on that trial to be 3 ms.
...
timing variability within each song and across songs (which may be
responsible for some of our measured jitter).

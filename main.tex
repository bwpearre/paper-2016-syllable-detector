...
\begin{abstract}
  ...
  well under 1\% and 0.001\% respectively. The detector can run on a
...
example, if the target trigger point is at 200 ms with respect to the
corpus of aligned songs, and for a given presentation of the song the
detector fires at 203 ms, we take the latency on that trial to be 3
ms.
...
\noprint{
  The means of the false positive and
  ...
  \begin{tabular}{r|cc}
    ...
    & pos & neg \\ 
    \hline  Detected pos & 99.34\% & 0.0015\%\\ 
    neg & 0.0355\% & 99.99970\%\\ 
  \end{tabular}
  \vspace{8pt}\par\noindent
  \begin{tabular}{r|cc}
...
chart in \fig{fig:accuracies}, and the means are shown in Table~\ref{table:accuracies}.  Evidently some syllables are easier to identify
reliably than others; thus, if the experimental design permits, it can be worthwhile to choose an easily identifiable syllable.  Furthermore, as can be seen from \fig{fig:accuracies}, for difficult syllables the training process
occasionally yields a network that performs unusually poorly.  It
